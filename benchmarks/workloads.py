...
def build_random(tokenizer=None, num_seqs: int = 256, min_input: int = 128,
                 max_input: int = 1024, output_len: int = 256, seed: int = 0,
                 **_) -> Workload:
    ...
    return Workload("random", "throughput", prompts, sp,
                    meta={"num_seqs": num_seqs, "output_len": output_len})


def build_sharegpt(tokenizer, num_prompts: int = 512, min_turns: int = 4,
                   max_turns: int = 6, max_prompt_tokens: int = 2048,
                   output_len: int = 64, seed: int = 0, dataset: str | None = None,
                   **_) -> Workload:
    from nanovllm.utils.sharegpt import (
        DEFAULT_SHAREGPT_PATH, build_multiturn_prompts, ensure_sharegpt,
        load_conversations,
    )
    ...
    prompts = build_multiturn_prompts(
        convs, tokenizer, max_turns=max_turns,
        max_prompt_tokens=max_prompt_tokens, max_prompts=num_prompts,
    )
    random.Random(seed).shuffle(prompts)
    sp = SamplingParams(temperature=1.0, max_tokens=output_len, ignore_eos=True)
    return Workload("sharegpt", "prefix", prompts, sp, passes=2,
                    meta={"n_prompts": len(prompts), "output_len": output_len})


def build_longprefix(tokenizer=None, shared_prefix: int = 2048, num_prompts: int = 128,
                     suffix: int = 32, output_len: int = 64, seed: int = 0, **_) -> Workload:
    ...
    sp = SamplingParams(temperature=1.0, max_tokens=output_len, ignore_eos=True)
    return Workload("longprefix", "prefix", prompts, sp, passes=2,
                    meta={"shared_prefix": shared_prefix, "num_prompts": num_prompts})


def build_latency(tokenizer=None, prompt_len: int = 512, output_len: int = 128,
                  seed: int = 0, **_) -> Workload:
    rnd = random.Random(seed)
    prompts = [[rnd.randint(0, SYNTH_VOCAB - 1) for _ in range(prompt_len)]]
    sp = SamplingParams(temperature=1.0, max_tokens=output_len, ignore_eos=True)
    return Workload("latency", "latency", prompts, sp,
                    meta={"prompt_len": prompt_len, "output_len": output_len})


def build_gsm8k(tokenizer, num_questions: int = 200, output_len: int = 512,
                seed: int = 0, **_) -> Workload:
    ...
    return Workload("gsm8k", "accuracy", prompts, sp, references=golds,
